...
(
  ...
  input logic  [`OP_BITS-1:0]               sche_in_a_opcode_i     ,
  input logic  [`SIZE_BITS-1:0]             sche_in_a_size_i       ,
  input logic  [`SOURCE_BITS-1:0]           sche_in_a_source_i     ,
  input logic  [`ADDRESS_BITS-1:0]          sche_in_a_addresss_i   ,
  input logic  [`MASK_BITS-1:0]             sche_in_a_mask_i       ,
  input logic  [`DATA_BITS-1:0]             sche_in_a_data_i       ,
  input logic  [`PARAM_BITS-1:0]            sche_in_a_param_i      ,
  
  // in_d part Decoupled and its handshake sigansls
  output logic                              sche_in_d_valid_o      ,
  input logic                               sche_in_d_ready_i      ,
  
  output logic [`ADDRESS_BITS-1:0]          sche_in_d_address_o    ,
  output logic [`OP_BITS-1:0]               sche_in_d_opcode_o     ,
  output logic [`SIZE_BITS-1:0]             sche_in_d_size_o       ,
  output logic [`SOURCE_BITS-1:0]           sche_in_d_source_o     ,
  output logic [`DATA_BITS-1:0]             sche_in_d_data_o       ,
  output logic [`PARAM_BITS-1:0]            sche_in_d_param_o      ,
  ...
  output logic [`OP_BITS-1:0]               sche_out_a_opcode_o    ,
  output logic [`SIZE_BITS-1:0]             sche_out_a_size_o      ,
  output logic [`SOURCE_BITS-1:0]           sche_out_a_source_o    ,
  output logic [`ADDRESS_BITS-1:0]          sche_out_a_addresss_o  ,
  output logic [`MASK_BITS-1:0]             sche_out_a_mask_o      ,
  output logic [`DATA_BITS-1:0]             sche_out_a_data_o      ,
  output logic [`PARAM_BITS-1:0]            sche_out_a_param_o     ,
  
  // out_d part Decoupled and its handshake sigansls
  input logic                               sche_out_d_valid_i     ,
  output logic                              sche_out_d_ready_o     ,
  input logic  [`OP_BITS-1:0]               sche_out_d_opcode_i    ,
  //input logic  [`SIZE_BITS-1:0]             sche_out_d_size_i      ,
  input logic  [`SOURCE_BITS-1:0]           sche_out_d_source_i    ,
  input logic  [`DATA_BITS-1:0]             sche_out_d_data_i      
  //input logic  [`PARAM_BITS-1:0]            sche_out_d_param_i     
  );
  ...
  logic  [`SET_BITS-1:0]                sourceA_req_set_i        ;
  //logic  [`L2C_BITS-1:0]                sourceA_req_l2cidx_i     ;
  logic  [`OP_BITS-1:0]                 sourceA_req_opcode_i     ;
  logic  [`SIZE_BITS-1:0]               sourceA_req_size_i       ;
  logic  [`SOURCE_BITS-1:0]             sourceA_req_source_i     ;
  logic  [`TAG_BITS-1:0]                sourceA_req_tag_i        ;
  logic  [`OFFSET_BITS-1:0]             sourceA_req_offset_i     ;
  logic  [`PUT_BITS-1:0]                sourceA_req_put_i        ;
  logic  [`DATA_BITS-1:0]               sourceA_req_data_i       ;
  logic  [`MASK_BITS-1:0]               sourceA_req_mask_i       ;
  logic  [`PARAM_BITS-1:0]              sourceA_req_param_i      ;
  //req part
  logic                                 sourceA_a_ready_i        ;
  logic                                 sourceA_a_valid_o        ;
  //a part decoupled 
  logic  [`OP_BITS-1:0]                 sourceA_a_opcode_o       ;
  logic  [`SIZE_BITS-1:0]               sourceA_a_size_o         ;
  logic  [`SOURCE_BITS-1:0]             sourceA_a_source_o       ;
  logic  [`ADDRESS_BITS-1:0]            sourceA_a_address_o      ;
  logic  [`MASK_BITS-1:0]               sourceA_a_mask_o         ;
  logic  [`DATA_BITS-1:0]               sourceA_a_data_o         ;
  logic  [`PARAM_BITS-1:0]              sourceA_a_param_o        ;
  //above part is the Source A
  logic                                 SourceD_req_from_mem_i   ;
  logic                                 SourceD_req_hit_i        ;
  logic  [`WAY_BITS-1:0]                SourceD_req_way_i        ;
  logic                                 SourceD_req_dirty_i      ;
  logic                                 SourceD_req_flush_i      ;
  logic                                 SourceD_req_last_flush_i ;
  logic  [`SET_BITS-1:0]                SourceD_req_set_i        ;
  //logic  [`L2C_BITS-1:0]                SourceD_req_l2cidx_i     ;
  logic  [`OP_BITS-1:0]                 SourceD_req_opcode_i     ;
  logic  [`SIZE_BITS-1:0]               SourceD_req_size_i       ;
  logic  [`SOURCE_BITS-1:0]             SourceD_req_source_i     ;
  logic  [`TAG_BITS-1:0]                SourceD_req_tag_i        ;
  logic  [`OFFSET_BITS-1:0]             SourceD_req_offset_i     ;
  logic  [`PUT_BITS-1:0]                SourceD_req_put_i        ;
  logic  [`DATA_BITS-1:0]               SourceD_req_data_i       ;
  logic  [`MASK_BITS-1:0]               SourceD_req_mask_i       ;
  logic  [2:0]                          SourceD_req_param_i      ;
  logic                                 SourceD_req_valid_i      ;
  logic                                 SourceD_req_ready_o      ;
  logic  [`ADDRESS_BITS-1:0]            SourceD_d_address_o      ;
  logic  [`OP_BITS-1:0]                 SourceD_d_opcode_o       ;
  logic  [`SIZE_BITS-1:0]               SourceD_d_size_o         ;
  logic  [`SOURCE_BITS-1:0]             SourceD_d_source_o       ;
  logic  [`DATA_BITS-1:0]               SourceD_d_data_o         ;
  logic  [2:0]                          SourceD_d_param_o        ;
  logic                                 SourceD_d_valid_o        ;
  logic                                 SourceD_d_ready_i        ;
  logic  [`PUT_BITS-1:0]                SourceD_pb_pop_index_o   ;
  logic                                 SourceD_pb_pop_valid_o   ;
  logic                                 SourceD_pb_pop_ready_i   ;
  logic  [`DATA_BITS-1:0]               SourceD_pb_beat_data_i   ;
  logic  [`MASK_BITS-1:0]               SourceD_pb_beat_mask_i   ;
  logic  [`WAY_BITS-1:0]                SourceD_bs_radr_way_o    ;
  logic  [`SET_BITS-1:0]                SourceD_bs_radr_set_o    ;
  logic  [`INNER_MASK_BITS-1:0]         SourceD_bs_radr_mask_o   ;
  logic                                 SourceD_bs_radr_valid_o  ;
  logic                                 SourceD_bs_radr_ready_i  ;
  logic  [`L2CACHE_BEATBYTES*8-1:0]     SourceD_bs_rdat_data_i   ;
  logic  [`WAY_BITS-1:0]                SourceD_bs_wadr_way_o    ;
  logic  [`SET_BITS-1:0]                SourceD_bs_wadr_set_o    ;
  logic  [`INNER_MASK_BITS-1:0]         SourceD_bs_wadr_mask_o   ;
  logic                                 SourceD_bs_wadr_valid_o  ;
  logic                                 SourceD_bs_wadr_ready_i  ;
  logic  [`L2CACHE_BEATBYTES*8-1:0]     SourceD_bs_wdat_data_o   ;
  logic  [`SET_BITS-1:0]                SourceD_a_set_o          ;
  //logic  [`L2C_BITS-1:0]                SourceD_a_l2cidx_o       ;
  logic  [`OP_BITS-1:0]                 SourceD_a_opcode_o       ;
  logic  [`SIZE_BITS-1:0]               SourceD_a_size_o         ;
  logic  [`SOURCE_BITS-1:0]             SourceD_a_source_o       ;
  logic  [`TAG_BITS-1:0]                SourceD_a_tag_o          ;
  logic  [`OFFSET_BITS-1:0]             SourceD_a_offset_o       ;
  logic  [`PUT_BITS-1:0]                SourceD_a_put_o          ;
  logic  [`DATA_BITS-1:0]               SourceD_a_data_o         ;
  logic  [`MASK_BITS-1:0]               SourceD_a_mask_o         ;
  ...
  logic  [`SET_BITS-1:0]                sinkA_req_set_o          ;
  //logic  [`L2C_BITS-1:0]                sinkA_req_l2cidx_o       ;
  logic  [`OP_BITS-1:0]                 sinkA_req_opcode_o       ;
  logic  [`SIZE_BITS-1:0]               sinkA_req_size_o         ;
  logic  [`SOURCE_BITS-1:0]             sinkA_req_source_o       ;
  logic  [`TAG_BITS-1:0]                sinkA_req_tag_o          ;
  logic  [`OFFSET_BITS-1:0]             sinkA_req_offset_o       ;
  logic  [`PUT_BITS-1:0]                sinkA_req_put_o          ;
  logic  [`DATA_BITS-1:0]               sinkA_req_data_o         ;
  logic  [`MASK_BITS-1:0]               sinkA_req_mask_o         ;
  logic  [`PARAM_BITS-1:0]              sinkA_req_param_o        ;
  //Flipped(Decoupled -TLBundleA_lite   
  logic                                 sinkA_a_ready_o          ;
  logic                                 sinkA_a_valid_i          ;
  //sinkA a part handshake signals
  logic   [`OP_BITS-1:0]                sinkA_a_opcode_i         ;
  logic   [`SIZE_BITS-1:0]              sinkA_a_size_i           ;
  logic   [`SOURCE_BITS-1:0]            sinkA_a_source_i         ;
  logic   [`ADDRESS_BITS-1:0]           sinkA_a_address_i        ;
  logic   [`MASK_BITS-1:0]              sinkA_a_mask_i           ;
  logic   [`DATA_BITS-1:0]              sinkA_a_data_i           ;
  logic   [`PARAM_BITS-1:0]             sinkA_a_param_i          ;
  logic                                 sinkA_pb_pop_ready_o     ;
  logic                                 sinkA_pb_pop_valid_i     ;
  logic   [`PUT_BITS-1:0]               sinkA_pb_pop_index_i     ;
  logic   [`DATA_BITS-1:0]              sinkA_pb_beat_data_o     ;
  logic   [`MASK_BITS-1:0]              sinkA_pb_beat_mask_o     ;
  logic                                 sinkA_empty_o            ;
  //above is the sinkA port
  logic   [`OP_BITS-1:0]                sinkD_d_opcode_i         ;
  logic   [`SOURCE_BITS-1:0]            sinkD_d_source_i         ;
  logic   [`DATA_BITS-1:0]              sinkD_d_data_i           ;
  logic                                 sinkD_d_valid_i          ;
  logic                                 sinkD_d_ready_o          ;
  //logic       [`PUT_BITS-1:0]            sinkD_put_i            ;
  //logic      [`PUT_BITS-1:0]            sinkD_index_o          ;
  logic   [`SOURCE_BITS-1:0]            sinkD_source_o           ;
  logic   [`OP_BITS-1:0]                sinkD_resp_opcode_o      ;
  logic   [`SOURCE_BITS-1:0]            sinkD_resp_source_o      ;
  logic   [`DATA_BITS-1:0]              sinkD_resp_data_o        ;
  logic                                 sinkD_resp_valid_o       ;
  //above is the sinkD part
  //write port
  logic                                 dir_write_valid_i        ;
  logic                                 dir_write_ready_o        ;
  logic [`WAY_BITS-1:0]                 dir_write_way_i          ;
  //tag is the write data
  logic [`TAG_BITS-1:0]                 dir_write_tag_i          ;
  logic [`SET_BITS-1:0]                 dir_write_set_i          ;
  //read port
  logic                                 dir_read_valid_i         ;
  logic                                 dir_read_ready_o         ;
  logic [`SET_BITS-1:0]                 dir_read_set_i           ;
  //logic [`L2C_BITS-1:0]                 dir_read_l2cidx_i        ;
  logic [`OP_BITS-1:0]                  dir_read_opcode_i        ;
  logic [`SIZE_BITS-1:0]                dir_read_size_i          ;
  logic [`SOURCE_BITS-1:0]              dir_read_source_i        ;
  logic [`TAG_BITS-1:0]                 dir_read_tag_i           ;
  logic [`OFFSET_BITS-1:0]              dir_read_offset_i        ;
  logic [`PUT_BITS-1:0]                  dir_read_put_i          ;
  logic [`DATA_BITS-1:0]                dir_read_data_i          ;
  logic [`MASK_BITS-1:0]                dir_read_mask_i          ;
  logic [`PARAM_BITS-1:0]               dir_read_param_i         ;
  //result port
  logic                                 dir_result_valid_o       ;
  logic                                 dir_result_ready_i       ;
  logic [`TAG_BITS-1:0]                 dir_result_victim_tag_o  ;
  logic [`WAY_BITS-1:0]                 dir_result_way_o         ;
  logic                                 dir_result_hit_o         ;
  logic                                 dir_result_dirty_o       ;
  logic                                 dir_result_flush_o       ;
  logic                                 dir_result_last_flush_o  ;
  logic [`SET_BITS-1:0]                 dir_result_set_o         ;
  //logic [`L2C_BITS-1:0]                 dir_result_l2cidx_o      ;
  logic [`OP_BITS-1:0]                  dir_result_opcode_o      ;
  logic [`SIZE_BITS-1:0]                dir_result_size_o        ;
  logic [`SOURCE_BITS-1:0]              dir_result_source_o      ;
  logic [`TAG_BITS-1:0]                 dir_result_tag_o         ;
  logic [`OFFSET_BITS-1:0]              dir_result_offset_o      ;
  logic [`PUT_BITS-1:0]                  dir_result_put_o        ;
  logic [`DATA_BITS-1:0]                dir_result_data_o        ;
  logic [`MASK_BITS-1:0]                dir_result_mask_o        ;
  logic [`PARAM_BITS-1:0]               dir_result_param_o       ;
  //ready port
  logic                                 dir_ready_o              ;
  //flush port
  logic                                 dir_flush_i              ;
  ...
  logic   [`WAY_BITS-1:0]               bank_s_sinkD_adr_way_i     ;
  logic   [`SET_BITS-1:0]               bank_s_sinkD_adr_set_i     ;
  logic   [`OUTER_MASK_BITS-1:0]        bank_s_sinkD_adr_mask_i    ;
  logic   [`L2CACHE_BEATBYTES*8-1:0]    bank_s_sinkD_dat_data_i    ;
  logic                                 bank_s_sinkD_adr_valid_i   ;
  logic                                 bank_s_sinkD_adr_ready_o   ;
  logic   [`WAY_BITS-1:0]               bank_s_sourceD_radr_way_i  ;
  logic   [`SET_BITS-1:0]               bank_s_sourceD_radr_set_i  ;
  logic   [`INNER_MASK_BITS-1:0]        bank_s_sourceD_radr_mask_i ;
  logic  [`L2CACHE_BEATBYTES*8-1:0]     bank_s_sourceD_rdat_data_o ;
  logic                                 bank_s_sourceD_radr_valid_i;
  logic                                 bank_s_sourceD_radr_ready_o;
  logic   [`WAY_BITS-1:0]               bank_s_sourceD_wadr_way_i  ;
  logic   [`SET_BITS-1:0]               bank_s_sourceD_wadr_set_i  ;
  logic   [`INNER_MASK_BITS-1:0]        bank_s_sourceD_wadr_mask_i ;
  logic   [`L2CACHE_BEATBYTES*8-1:0]    bank_s_sourceD_wdat_data_i ;
  ...
  logic [`PUT_BITS-1:0]                 requests_push_index_i      ;//data in
  //logic [`PUTLISTS-1:0]                 requests_push_index_req    ;
  logic [`DATA_BITS-1:0]                requests_push_data_data_i  ;//data in;
  logic [`MASK_BITS-1:0]                requests_push_data_mask_i  ;//data in;
  logic [`PUT_BITS-1:0]                 requests_push_data_put_i   ;
  logic [`OP_BITS-1:0]                  requests_push_data_opcode_i;
  logic [`SOURCE_BITS-1:0]              requests_push_data_source_i;
  logic [`PUTLISTS-1:0]                 requests_valid_o           ;//data out
  logic                                 requests_pop_valid_i       ;//data in
  logic [`PUT_BITS-1:0]                 requests_pop_data_i        ;//data in
  logic [`DATA_BITS-1:0]                requests_data_data_o       ;//data out
  logic [`MASK_BITS-1:0]                requests_data_mask_o       ;//data out
  logic [`PUT_BITS-1:0]                 requests_data_put_o        ;
  logic [`OP_BITS-1:0]                  requests_data_opcode_o     ;
  logic [`SOURCE_BITS-1:0]              requests_data_source_o     ;
  //above is the listbuffer port
  //logic allocted signals
  //below is the MSHR instance part
  logic  [`MSHRS-1:0             ]      mshr_alloc_valid_i          ;
  logic  [`MSHRS-1:0             ]      mshr_alloc_hit_i            ;
  logic  [`MSHRS*`WAY_BITS-1:0   ]      mshr_alloc_way_i            ;
  logic  [`MSHRS-1:0             ]      mshr_alloc_dirty_i          ;
  logic  [`MSHRS-1:0             ]      mshr_alloc_flush_i          ;
  logic  [`MSHRS-1:0             ]      mshr_alloc_last_flush_i     ;
  logic  [`MSHRS*`SET_BITS-1:0   ]      mshr_alloc_set_i            ;
  //logic [`MSHRS*`L2C_BITS-1:0  ]      mshr_alloc_l2cidx_i         ;
  logic  [`MSHRS*`OP_BITS-1:0    ]      mshr_alloc_opcode_i         ;
  logic  [`MSHRS*`SIZE_BITS-1:0  ]      mshr_alloc_size_i           ;
  logic  [`MSHRS*`SOURCE_BITS-1:0]      mshr_alloc_source_i         ;
  logic  [`MSHRS*`TAG_BITS-1:0   ]      mshr_alloc_tag_i            ;
  logic  [`MSHRS*`OFFSET_BITS-1:0]      mshr_alloc_offset_i         ;
  logic  [`MSHRS*`PUT_BITS-1:0   ]      mshr_alloc_put_i            ;
  logic  [`MSHRS*`DATA_BITS-1:0  ]      mshr_alloc_data_i           ;
  logic  [`MSHRS*`MASK_BITS-1:0  ]      mshr_alloc_mask_i           ;
  logic  [`MSHRS*`PARAM_BITS-1:0 ]      mshr_alloc_param_i          ;
  logic  [`MSHRS-1:0             ]      mshr_status_hit_o           ;
  logic  [`MSHRS*`WAY_BITS-1:0   ]      mshr_status_way_o           ;
  logic  [`MSHRS-1:0             ]      mshr_status_dirty_o         ;
  logic  [`MSHRS-1:0             ]      mshr_status_flush_o         ;
  logic  [`MSHRS-1:0             ]      mshr_status_last_flush_o    ;
  logic  [`MSHRS*`SET_BITS-1:0   ]      mshr_status_set_o           ;
  //logic  [`MSHRS*`L2C_BITS-1:0 ]      mshr_status_l2cidx_o        ;
  logic  [`MSHRS*`OP_BITS-1:0    ]      mshr_status_opcode_o        ;
  logic  [`MSHRS*`SIZE_BITS-1:0  ]      mshr_status_size_o          ;
  logic  [`MSHRS*`SOURCE_BITS-1:0]      mshr_status_source_o        ;
  logic  [`MSHRS*`TAG_BITS-1:0   ]      mshr_status_tag_o           ;
  logic  [`MSHRS*`OFFSET_BITS-1:0]      mshr_status_offset_o        ;
  logic  [`MSHRS*`PUT_BITS-1:0   ]      mshr_status_put_o           ;
  logic  [`MSHRS*`DATA_BITS-1:0  ]      mshr_status_data_o          ;
  logic  [`MSHRS*`MASK_BITS-1:0  ]      mshr_status_mask_o          ;
  logic  [`MSHRS*`PARAM_BITS-1:0 ]      mshr_status_param_o         ;
  logic  [`MSHRS-1:0             ]      mshr_valid_i                ;
  logic  [`MSHRS-1:0             ]      mshr_wait_i                 ;
  logic  [`MSHRS-1:0             ]      mshr_mixed_i                ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_a_ready_i     ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_a_valid_o     ;
  logic  [`MSHRS*`SET_BITS-1:0   ]      mshr_schedule_a_set_o       ;
  //logic  [`MSHRS*`L2C_BITS-1:0 ]      mshr_schedule_a_l2cidx_o    ;
  logic  [`MSHRS*`OP_BITS-1:0    ]      mshr_schedule_a_opcode_o    ;
  logic  [`MSHRS*`SIZE_BITS-1:0  ]      mshr_schedule_a_size_o      ;
  logic  [`MSHRS*`SOURCE_BITS-1:0]      mshr_schedule_a_source_o    ;
  logic  [`MSHRS*`TAG_BITS-1:0   ]      mshr_schedule_a_tag_o       ;
  logic  [`MSHRS*`OFFSET_BITS-1:0]      mshr_schedule_a_offset_o    ;
  logic  [`MSHRS*`PUT_BITS-1:0   ]      mshr_schedule_a_put_o       ;
  logic  [`MSHRS*`DATA_BITS-1:0  ]      mshr_schedule_a_data_o      ;
  logic  [`MSHRS*`MASK_BITS-1:0  ]      mshr_schedule_a_mask_o      ;
  logic  [`MSHRS*`PARAM_BITS-1:0 ]      mshr_schedule_a_param_o     ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_d_ready_i     ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_d_valid_o     ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_d_hit_o       ;
  logic  [`MSHRS*`WAY_BITS-1:0   ]      mshr_schedule_d_way_o       ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_d_dirty_o     ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_d_flush_o     ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_d_last_flush_o;
  logic  [`MSHRS*`SET_BITS-1:0   ]      mshr_schedule_d_set_o       ;
  logic  [`MSHRS*`OP_BITS-1:0    ]      mshr_schedule_d_opcode_o    ;
  logic  [`MSHRS*`SIZE_BITS-1:0  ]      mshr_schedule_d_size_o      ;
  logic  [`MSHRS*`SOURCE_BITS-1:0]      mshr_schedule_d_source_o    ;
  logic  [`MSHRS*`TAG_BITS-1:0   ]      mshr_schedule_d_tag_o       ;
  logic  [`MSHRS*`OFFSET_BITS-1:0]      mshr_schedule_d_offset_o    ;
  logic  [`MSHRS*`PUT_BITS-1:0   ]      mshr_schedule_d_put_o       ;
  logic  [`MSHRS*`DATA_BITS-1:0  ]      mshr_schedule_d_data_o      ;
  logic  [`MSHRS*`MASK_BITS-1:0  ]      mshr_schedule_d_mask_o      ;
  logic  [`MSHRS*`PARAM_BITS-1:0 ]      mshr_schedule_d_param_o     ; 
  logic  [`MSHRS*`DATA_BITS-1:0  ]      mshr_schedule_data_o        ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_dir_ready_i   ;
  logic  [`MSHRS-1:0             ]      mshr_schedule_dir_valid_o   ;
  logic  [`MSHRS*`WAY_BITS-1:0   ]      mshr_schedule_dir_way_o     ;
  logic  [`MSHRS*`TAG_BITS-1:0   ]      mshr_schedule_dir_data_tag_o;
  logic  [`MSHRS*`SET_BITS-1:0   ]      mshr_schedule_dir_set_o     ;
  logic  [`MSHRS-1:0             ]      mshr_merge_valid_i          ;
  logic  [`MSHRS-1:0             ]      mshr_merge_ready            ;
  logic  [`MSHRS*`MASK_BITS-1:0  ]      mshr_merge_mask_i           ;
  logic  [`MSHRS*`DATA_BITS-1:0  ]      mshr_merge_data_i           ;    
  logic  [`MSHRS*`OP_BITS-1:0    ]      mshr_merge_opcode_i         ;
  logic  [`MSHRS*`PUT_BITS-1:0   ]      mshr_merge_put_i            ;
  logic  [`MSHRS*`SOURCE_BITS-1:0]      mshr_merge_source_i         ;
  logic  [`MSHRS-1:0             ]      mshr_sinked_valid_i         ;
  logic  [`MSHRS*`OP_BITS-1:0    ]      mshr_sinked_opcode_i        ;
  logic  [`MSHRS*`SOURCE_BITS-1:0]      mshr_sinked_source_i        ;
  logic  [`MSHRS*`DATA_BITS-1:0  ]      mshr_sinked_data_i          ;
  //above is the MSHR part
  //val request = Wire(Decoupled(new FullRequest(params)))
  logic                                 request_ready_i             ;
  logic                                 request_valid_o             ;
  //sourceA req part handshake signals
  logic  [`SET_BITS-1:0]                request_set_o               ;
  logic  [`L2C_BITS-1:0]                request_l2cidx_o            ;
  logic  [`OP_BITS-1:0]                 request_opcode_o            ;
  logic  [`SIZE_BITS-1:0]               request_size_o              ;
  logic  [`SOURCE_BITS-1:0]             request_source_o            ;
  logic  [`TAG_BITS-1:0]                request_tag_o               ;
  logic  [`OFFSET_BITS-1:0]             request_offset_o            ;
  logic  [`PUT_BITS-1:0]                request_put_o               ;
  logic  [`DATA_BITS-1:0]               request_data_o              ;
  logic  [`MASK_BITS-1:0]               request_mask_o              ;
  logic  [`PARAM_BITS-1:0]              request_param_o             ;
  logic                                  issue_flush_invalidate      ; //init 0
  logic  [`MSHRS-1 : 0]                 mshr_request                ;
  //schedule and its definination
  logic                                  schedule_a_ready_i          ;
  logic                                  schedule_a_valid_o          ;
  //logic  schedule part handshake signals
  logic  [`SET_BITS-1:0]                 schedule_a_set_o            ;
  //logic  [`L2C_BITS-1:0]               schedule_a_l2cidx_o         ;
  logic  [`OP_BITS-1:0]                  schedule_a_opcode_o         ;
  logic  [`SIZE_BITS-1:0]                schedule_a_size_o           ;
  logic  [`SOURCE_BITS-1:0]              schedule_a_source_o         ;
  logic  [`TAG_BITS-1:0]                 schedule_a_tag_o            ;
  logic  [`OFFSET_BITS-1:0]              schedule_a_offset_o         ;
  logic  [`PUT_BITS-1:0]                 schedule_a_put_o            ;
  logic  [`DATA_BITS-1:0]                schedule_a_data_o           ;
  logic  [`MASK_BITS-1:0]                schedule_a_mask_o           ;
  logic  [`PARAM_BITS-1:0]               schedule_a_param_o          ;
  ...
  logic  [`WAY_BITS-1:0]                 schedule_d_way_o            ;
  logic                                  schedule_d_dirty_o          ;
  logic                                  schedule_d_flush_o          ;
  logic                                  schedule_d_last_flush_o     ;
  logic  [`SET_BITS-1:0]                 schedule_d_set_o            ;
  //logic  [`L2C_BITS-1:0]                 schedule_d_l2cidx_o         ;
  logic  [`OP_BITS-1:0]                  schedule_d_opcode_o         ;
  logic  [`SIZE_BITS-1:0]                schedule_d_size_o           ;
  logic  [`SOURCE_BITS-1:0]              schedule_d_source_o         ;
  logic  [`TAG_BITS-1:0]                 schedule_d_tag_o            ;
  logic  [`OFFSET_BITS-1:0]              schedule_d_offset_o         ;
  logic  [`PUT_BITS-1:0]                 schedule_d_put_o            ;
  logic  [`DATA_BITS-1:0]                schedule_d_data_o           ;
  logic  [`MASK_BITS-1:0]                schedule_d_mask_o           ;
  logic  [`PARAM_BITS-1:0]               schedule_d_param_o          ;
  //schedule  data part   
  logic  [`DATA_BITS-1:0]                schedule_data_o             ;
  //logic  schedule dir part
  logic                                  schedule_dir_ready_i        ;
  logic                                  schedule_dir_valid_o        ;
  //schedule part dir handshake signals
  logic  [`WAY_BITS-1:0]                 schedule_dir_way_o          ;
  logic  [`TAG_BITS-1:0]                 schedule_dir_data_tag_o     ;
  logic  [`SET_BITS-1:0]                 schedule_dir_set_o          ;
  logic                                 mshr_free                       ;
  logic                                 mshr_empty                      ;
  logic                                 putbuffer_empty                 ;
  logic                                 invalidate_ready                ;
  logic [`MSHRS-1:0]                    mshr_insertOH_zip_mshrs         ;
  logic [`PUTLISTS-1:0]                 mshr_validOH                    ;
  ...
  logic  [`SET_BITS-1:0]                writebuffer_enq_set             ;
  logic  [`SET_BITS-1:0]                writebuffer_deq_set             ;
  //logic  [`L2C_BITS-1:0]                writebuffer_enq_l2cidx          ;
  //logic  [`L2C_BITS-1:0]                writebuffer_deq_l2cidx          ;
  logic  [`OP_BITS-1:0]                 writebuffer_enq_opcode          ;
  logic  [`OP_BITS-1:0]                 writebuffer_deq_opcode          ;
  logic  [`SIZE_BITS-1:0]               writebuffer_enq_size            ;
  logic  [`SIZE_BITS-1:0]               writebuffer_deq_size            ;
  logic  [`SOURCE_BITS-1:0]             writebuffer_enq_source          ;
  logic  [`SOURCE_BITS-1:0]             writebuffer_deq_source          ;
  logic  [`TAG_BITS-1:0]                writebuffer_enq_tag             ;
  logic  [`TAG_BITS-1:0]                writebuffer_deq_tag             ;
  logic  [`OFFSET_BITS-1:0]             writebuffer_enq_offset          ;
  logic  [`OFFSET_BITS-1:0]             writebuffer_deq_offset          ;
  logic  [`PUT_BITS-1:0]                writebuffer_enq_put             ;
  logic  [`PUT_BITS-1:0]                writebuffer_deq_put             ;
  logic  [`DATA_BITS-1:0]               writebuffer_enq_data            ;
  logic  [`DATA_BITS-1:0]               writebuffer_deq_data            ;
  logic  [`MASK_BITS-1:0]               writebuffer_enq_mask            ;
  logic  [`MASK_BITS-1:0]               writebuffer_deq_mask            ;
  logic  [`PARAM_BITS-1:0]              writebuffer_enq_param           ;
  logic  [`PARAM_BITS-1:0]              writebuffer_deq_param           ;
  logic  [writebuffer_data_in_width-1:0]writebuffer_data_in             ;
  logic  [writebuffer_data_in_width-1:0]writebuffer_data_out            ;
  logic  [`MSHRS-1:0]                   tagMatches                      ;
  logic  [$clog2(`MSHRS)-1:0]           tagMatches2uint                 ;//tagMatches2uint is the one hot code tagMatches to uint code.
  logic                                 is_pending                      ;
  logic                                 alloc                           ;
  logic                                 pending_index                   ;//val pending_index = OHToUInt(Mux(is_pending,tagMatches,0.U))
  logic  [`PUTLISTS-1 :0]               mshr_insertOH_init,mshr_insertOH;//width is `PUTLISTS-1 + 1.
  logic  [$clog2(`PUTLISTS)-1:0]        mshr_insertOH2uint              ;
  ...
  logic   [`MSHRS-1:0]                   robin_filter                    ;//init 0
  logic  [2*`MSHRS-1:0]                 robin_request                   ;
  logic  [$clog2(`MSHRS)-1:0]           mshr_select                     ;
  logic  [2*`MSHRS:0]                   mshr_selectOH2                  ;//width of its 2*`MSHRS -1 +1
  logic  [`MSHRS-1:0]                   mshr_selectOH                   ;
