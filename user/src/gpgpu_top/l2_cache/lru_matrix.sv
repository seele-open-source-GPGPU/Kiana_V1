...
module lru_matrix #(
  ...
  input logic                  update_entry_i    , // input logic update condition
  input logic  [WAY_DEPTH-1:0] update_index_i    , // input logic update wayId
  output logic [WAY_DEPTH-1:0] lru_index_o         // output logic replacement wayId
  );

  //logic   [NUM_WAY*NUM_WAY-1:0] matrix             ; // matrix[row(i), ,row(1),row(0)]
