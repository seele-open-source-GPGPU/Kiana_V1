...
module banked_store (
  ...
  input logic   [`WAY_BITS-1:0]                 sinkD_adr_way_i     ,
  input logic   [`SET_BITS-1:0]                 sinkD_adr_set_i     ,
  input logic   [`OUTER_MASK_BITS-1:0]          sinkD_adr_mask_i    ,
  input logic   [`L2CACHE_BEATBYTES*8-1:0]      sinkD_dat_data_i    ,
  input logic                                   sinkD_adr_valid_i   ,
  output logic                                  sinkD_adr_ready_o   ,

  input logic   [`WAY_BITS-1:0]                 sourceD_radr_way_i  ,
  input logic   [`SET_BITS-1:0]                 sourceD_radr_set_i  ,
  input logic   [`INNER_MASK_BITS-1:0]          sourceD_radr_mask_i ,
  output logic  [`L2CACHE_BEATBYTES*8-1:0]      sourceD_rdat_data_o ,
  input logic                                   sourceD_radr_valid_i,
  output logic                                  sourceD_radr_ready_o,

  input logic   [`WAY_BITS-1:0]                 sourceD_wadr_way_i  ,
  input logic   [`SET_BITS-1:0]                 sourceD_wadr_set_i  ,
  input logic   [`INNER_MASK_BITS-1:0]          sourceD_wadr_mask_i ,
  input logic   [`L2CACHE_BEATBYTES*8-1:0]      sourceD_wdat_data_i ,
  ...
  logic    [`SET_BITS-1:0]                           set_sel                        ;
  logic    [`WAY_BITS-1:0]                           way_sel                        ;
  logic    [`L2CACHE_NWAYS-1:0]                      waymask                        ;
  logic    [`L2CACHE_BEATBYTES*8-1:0]                data_sel                       ;
  logic    [CODE_BITS-1:0]                           bank_datain     [0:NUM_BANKS-1];
  logic    [`L2CACHE_NWAYS*CODE_BITS-1:0]            bank_set_datain [0:NUM_BANKS-1];             
  logic    [MASK_BITS-1:0]                           mask_sel                       ;
  logic    [NUM_BANKS-1:0]                           sram_template_wen              ;   
  logic    [CODE_BITS*`L2CACHE_NWAYS*NUM_BANKS-1:0]  sram_template_rdata            ;
  logic     [`INNER_MASK_BITS-1:0]                    sourceD_radr_mask_reg          ;
